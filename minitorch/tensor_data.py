...
def shape_broadcast(shape1, shape2):
    ...
    for dim1, dim2 in zip(shape1[::-1], shape2[::-1]):
        if dim1 == dim2:
            ...
            raise IndexingError(f"Filed to broadcast {shape1} and {shape2}")
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        # TODO: Implement for Task 2.1.
        ret_shape = tuple([self.shape[i] for i in order])
        ret_strides = tuple([self.strides[i] for i in order])
        return TensorData(self._storage, ret_shape, strides=ret_strides)
