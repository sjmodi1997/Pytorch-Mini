    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        for idx, var in enumerate(inputs):
            if not is_constant(var):
                try:
                    vars_with_derivatives.append((var, var.expand(derivatives[idx])))
                except IndexError:
                    print(inputs.__sizeof__(), derivatives.__sizeof__())

        return vars_with_derivatives
...
def backpropagate(variable, deriv):
    ...
    for var in order:
        map_var_derv[var.unique_id] = 0.0

    map_var_derv[variable.unique_id] = deriv
    for var in order:
        if var.is_leaf():
            var.accumulate_derivative(map_var_derv[var.unique_id])
        else:
            chain = var.history.backprop_step(map_var_derv[var.unique_id])
            for a, b in chain:
                map_var_derv[a.unique_id] += b
